[**@quojs/core**](../README.md)

***

[@quojs/core](../README.md) / types

# types

## Interfaces

- [Action](interfaces/Action.md)
- [Change](interfaces/Change.md)
- [ReducerSpec](interfaces/ReducerSpec.md)
- [StoreInstance](interfaces/StoreInstance.md)

## Type Aliases

- [ActionMapBase](type-aliases/ActionMapBase.md)
- [ActionPair](type-aliases/ActionPair.md)
- [ActionUnion](type-aliases/ActionUnion.md)
- [AMFromReducersStrict](type-aliases/AMFromReducersStrict.md)
- [Connect](type-aliases/Connect.md)
- [ConnectAny](type-aliases/ConnectAny.md)
- [ConnectDeep](type-aliases/ConnectDeep.md)
- [ConnectTopLevel](type-aliases/ConnectTopLevel.md)
- [DeepReadonly](type-aliases/DeepReadonly.md)
- [DeepRO](type-aliases/DeepRO.md)
- [Dispatch](type-aliases/Dispatch.md)
- [Dotted](type-aliases/Dotted.md)
- [EffectFunction](type-aliases/EffectFunction.md)
- [MiddlewareFunction](type-aliases/MiddlewareFunction.md)
- [Path](type-aliases/Path.md)
- [Primitive](type-aliases/Primitive.md)
- [ReducerFunction](type-aliases/ReducerFunction.md)
- [ReducersMapAny](type-aliases/ReducersMapAny.md)
- [StateFromReducers](type-aliases/StateFromReducers.md)
- [StoreSpec](type-aliases/StoreSpec.md)
- [TopLevelProp](type-aliases/TopLevelProp.md)
- [Unsubscribe](type-aliases/Unsubscribe.md)
- [WithGlob](type-aliases/WithGlob.md)

## Functions

- [freezeState](functions/freezeState.md)
